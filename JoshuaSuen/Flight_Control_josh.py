...
def GPS_Coords(far_field, heading, lat_ant, long_ant, alt_ant, number_points):
    ...
    for i in range(new_degree_left + degree,new_degree_left + (120 + degree) ,degree):	# Calculates the next 11 points with 12 degrees of change between each, and fills all 60 points into a list
        ...
        point_list_arc1[z+1] = Next_Point(far_field, next_angle, lat_ant, long_ant,alt_ant)     # Fills in list with desired number of GPS coordinates
        z = z + 1
...
antenna_length = raw_input("Antenna Length (Must be positive integer): ")        # MUST be an odd number, to satisfy 'range' used in GPS_Coords function


while not (antenna_length.isdigit()):                                #checks that input is an integer
    print("Input is not a positive integer")
    print(" ")
    antenna_length = raw_input("Antenna Length (Must be positive integer): ")
...
number_points = raw_input("Number of points (MUST be odd number): ")        # MUST be an odd number, to satisfy 'range' used in GPS_Coords function

while not (number_points.isdigit()):                                #checks that input is an integer
    print("Input is not a positive integer")
    print(" ")
    number_points = raw_input("Number of points? (MUST be odd number of 3 or more): ")

number_points = int(number_points)
while ((number_points < 2) | (number_points % 2 == 0)):                                            #Checks that an odd number of 3 points is input
    print("Input odd number of at least 3")
    number_points =  input("Number of points? (MUST be odd number of 3 or more): ")
number_points = int(number_points)
...
while vehicle.channels['5'] < 1200:		# If Flight Mode = 2 on controller, it is in autonomous mode
    ...
    print("Starting measurements for 1st point.")
    data.saveSignal(0, antenna_frequency, file_path)
